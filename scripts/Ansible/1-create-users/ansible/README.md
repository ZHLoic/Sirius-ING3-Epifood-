...
pour exec le playbook sur une partie seulement de l'inventaire:
```ssh
ansible-playbook -i list_servers.yml -u epifood -k -K -b playbook.yml -l '<groupe|host|expression>'
```

exemple
```ssh
#prendre que K8S
ansible-playbook -i list_servers.yml -u epifood -k -K -b playbook.yml -l 'K8S'

#prendre ce qui est dans NCC mais pas ces fils
ansible-playbook -i list_servers.yml -u epifood -k -K -b playbook.yml -l 'NCC:!K8S:!PG:!HA'
```